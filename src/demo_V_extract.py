...
def demo(opt):
  ...
  while True:
      ...
      if opt.save_video:
        ...
        k = 0
        for j in results[cnt]:
          if not os.path.exists('../results/{}'.format(out_name)):
            os.mkdir('../results/{}'.format(out_name))
          cv2.imwrite('../results/{}/demo{}_{}.jpg'.format(out_name, cnt, str(k)), img[int(j['bbox'][1]):int(j['bbox'][3]), int(j['bbox'][0]):int(j['bbox'][2])])
          k+=1
        if not is_video:
...
if __name__ == '__main__':
  opt = opts().init()
  Videos=os.listdir('/home/fyp3/Desktop/Batch18/Obj_Track/TraDeS/videos/weligama/')
  for vd in Videos:
    opt.demo='../videos/weligama/'+vd
    print("*************",opt.demo)
    demo(opt)
